...
describe("1. Initial Setup Tests", function() {
  ...
  before(async function() {
    ...
    wallets.TASK_MANAGER = await getWalletInfo(WALLET_IDS.TASK_MANAGER);
  ...
  it("should deploy TaskManager contract", async function() {
    ...
    taskManagerAddress = await deployTaskManager(
      ...
      parseEther("0.01"), // 0.01 cUSD per participant
      5n // 5 target participants
    );
    ...
    expect(owner.toLowerCase()).to.equal(wallets.TASK_MANAGER.serverWalletAccount.address.toLowerCase());
  });
  ...
  it("should verify PaxAccount initialization parameters", async function() {
    ...
    for (let i = 0; i < paxAccountAddresses.length; i++) {
      ...
      expect(primaryPaymentMethod.toLowerCase()).to.equal(wallet.address.toLowerCase());
    }
  });
  ...
  it("should verify TaskManager configuration", async function() {
    ...
    expect(rewardToken.toLowerCase()).to.equal(REWARD_TOKEN_ADDRESS.toLowerCase());
  });
